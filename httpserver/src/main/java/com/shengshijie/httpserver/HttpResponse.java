...
public class HttpResponse extends DefaultFullHttpResponse {
    ...
    private HttpResponse(HttpResponseStatus status, ByteBuf buffer ) {
    ...
    public static FullHttpResponse make(HttpResponseStatus status) {
        if (HttpResponseStatus.UNAUTHORIZED == status) {
            return HttpResponse.make(HttpResponseStatus.UNAUTHORIZED, CONTENT_ERROR_401);
        }
        if (HttpResponseStatus.NOT_FOUND == status) {
            return HttpResponse.make(HttpResponseStatus.NOT_FOUND, CONTENT_ERROR_404);
        }
        if (HttpResponseStatus.METHOD_NOT_ALLOWED == status) {
            return HttpResponse.make(HttpResponseStatus.METHOD_NOT_ALLOWED, CONTENT_ERROR_405);
        }
        return HttpResponse.make(HttpResponseStatus.OK,CONTENT_NORMAL_200);
    }
    ...
    public static FullHttpResponse makeError(Exception exception) {
        ...
        return HttpResponse.make(HttpResponseStatus.INTERNAL_SERVER_ERROR, String.format(CONTENT_ERROR_500,message));
    }
    ...
    private static FullHttpResponse make(HttpResponseStatus status,String content) {
        ...
        HttpResponse response = new HttpResponse(status,buffer);
